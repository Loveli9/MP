package com.hyp.utils.calculate_expression;
...
public class CalculateExpression {
     ...
     * 自定义表达式
     */
    private static final List<String> CUSTOMIZE_EXPS = Arrays.asList("sum", "count", "avg", "datadiff", "max", "min", "dateformat");

    @Test
    public void testSuffixExpression() {
        //com/hyp/utils/calculate_expression/expression.txt
        String exp = "min(test_b)+max(test_a)+min(test_a)+max(test_b)+count(test_a)+count(test_b)";
        DataManager dataManager = new DataManager();
        List<Object> testaList = new ArrayList<>();
        List<Object> testbList = new ArrayList<>();
        testaList.add(3);
        testaList.add(6);
        testaList.add(9);
        testbList.add(9);
        testbList.add(5);
        testbList.add(7);
        dataManager.put("test_a",testaList);
        dataManager.put("test_b",testbList);
        //转化为后缀表达式
        ...
        System.out.println(calculateResult.getData());
