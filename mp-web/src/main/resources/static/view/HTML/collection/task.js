;$(function () {
    $('#task_table').bootstrapTable({
        ...
        url: "http://192.168.0.11:8080/datacollection/collectiontask/query_simple",
        ...
        queryParams: function (params) {
            var param = {
                ...
                'params.projectId': getCookie("projNo_comm")
            }
            return param;
        },
    ...
    $("#interface_save_btn").on("click", function () {
        ...
        var select_ID = $('#task_table').bootstrapTable('getSelections')[0];
        var parameters = [];
        var item = {};
        editMsg.forEach(function (e) {
            item.name = e.name;
            item.value = e.value;
...
function add_editBtn(message) {
    ...
    if (message == "editMsg") {
        ...
        if (select_ID) {
            ...
            for (var i = 0; i < select_parameters.length; i++) {
                var cfgParameter = select_parameters[i];
                var exists = false;
                var item = {};
                for (var j = 0; j < select_table_data.length; j++) {
                    if (cfgParameter.name == select_table_data[j].name) {
                        exists = true;
                        item = select_table_data[j];
                        item.required = cfgParameter.required;
                        item.inputType = cfgParameter.inputType;
                        break
                    }
                }
                if (!exists) {
