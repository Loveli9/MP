;$(function () {
    $('#interface_table').bootstrapTable({
        ...
        url: "http://192.168.0.11:8080/datacollection/task_instance/query_done",
        ...
        queryParams: function (params) {
            var param = {
                ...
                'params.collectionTask.projectId': getCookie("projNo_comm"),
                "pageable.sort": '[{"property":"startTime","direction":"DESC"}]' //排位命令（desc，asc）
            }
            if ($.trim($("#queryName").val()).length > 0) {
                param["params.name_lk"] = $("#queryName").val()
            }
            return param;
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    var status = {1: "等执行", 2: "执行中", 3: "已完成", 4: "失败", 5: "异常", 6: "系统终止", 7: "用户终止"};
                    return status[value];
                }
            },
    ...
    $("#btn_log").on("click", function () {
        ...
        if (selections.length > 0) {
            ...
            $.ajax({
                url: 'http://10.28.1.252:8080/datacollection/executelog/page_by_task_instance_id',
                method: 'POST',
                ...
                success: function (resp) {
                    if (resp.success) {
                        ...
                        window.top.toastr.error(resp.data.result);
                    }
                }
            });
        } else {
            window.top.toastr.error("请先选择需要查看日志的任务！");
        }
    })
